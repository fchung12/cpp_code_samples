...
#define SHADE_MODE 1 //0 for flat, 1 for gouraud, otherwise phong
...
int Application5::Render()
{
	...
	*/
	while (fscanf(infile, "%s", dummy) == 1) { 	/* read in tri word */
		...
		phy_obj->ReadTriangle(3, nameListTriangle, valueListTriangle);
	}
	// Transforms model space gravity to image space
	phy_obj->setGrav(m_pRender);
...
//This is the softbody simulation loop, in Application5.cpp this runs in a while(true) and updates the main window
int Application5::Simulate(float elasedTime) {

	m_pRender->GzDefault();  /* init for new frame */
	//phy_obj->moveOneParticle();
	phy_obj->update(elasedTime);
	phy_obj->PutRendererTriangle(m_pRender);
